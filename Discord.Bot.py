...
async def add_rep(ctx, user: discord.User):  # TODO Probably going to disable the reputation system initially.
    rep = 0

    if user.id == ctx.author.id:
        await ctx.send("You cannot give yourself rep!")
        await add_rep.reset_cooldown(ctx)  # TODO FIX THIS WEIRD ERROR?
        return
...
async def add_rep_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        retry_time = time.strftime(r"%H:%M:%S", time.gmtime(error.retry_after))
        msg = 'This command is only available once a day, please try again in **{}**'.format(retry_time)
        await ctx.send(msg)
    else:
        raise error
